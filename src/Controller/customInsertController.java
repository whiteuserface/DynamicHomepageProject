...
@WebServlet("/custominsert")
public class customInsertController extends HttpServlet{
...
//		
		...
		try {
			if(country_eng != null && country_eng != "") {
				...
				country_eng = eng_kor[0]; // 첫번째것은 country_eng
				country_kor = eng_kor[1]; //18 두번째것은 country_kor로 넣는다.
			} 
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		String regi_info_man = request.getParameter("regi_info_man"); //23
		String regi_info_date = request.getParameter("regi_info_date"); //24
